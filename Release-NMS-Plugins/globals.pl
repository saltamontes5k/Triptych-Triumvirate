...
sub var
{
  ...
  else
  {
    ...
    for ($level = 1; $level < 10; $level++)
    {
      ...
      # caller() reports an eval block's subroutine name as the literal "(eval)". It is NOT a package
      # boundary, but the test below cannot tell: index() finds no ':' in "(eval)", so the prefix
      # computes to "(" and, being ne 'plugin::', STOPS the walk. Every plugin::val() called anywhere
      # inside an eval then resolves ${"(varname"} and returns undef -- silently, with no error until
      # something calls a method on it. Skip eval frames and keep stepping outward.
      my $subname = defined $context[3] ? $context[3] : '';
      next if $subname eq '(eval)';

      last if (($fullname = substr($subname, 0, index($subname, ':') + 2)) ne 'plugin::');
    }
