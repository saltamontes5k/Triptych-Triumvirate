		...
		switch(effect)
		{
...
#endif
						if (IsClient()) {
							// Multi-pet: a charmed pet can sit at any index, not just 0.
							// GetAllPets() returns a copy, so the charm-break inside the
							// fade (which erases from petids) can't invalidate the loop.
							if (RuleB(Spells, EvacClearCharmPet)) {
								for (auto evac_pet : GetAllPets()) {
									if (evac_pet->IsCharmed()) {
										evac_pet->BuffFadeByEffect(SE_Charm);
									}
								}
							}
			...
			case SE_ReclaimPet:
			{
				...
				caster->SetMana(caster->GetMana() + pet_ActSpellCost);
					...
					if(caster->IsClient())
						caster->RemovePet(this);  // THIS pet -- RemovePetByIndex() dropped
						                          // whatever sat at slot 0, which with
						                          // multiple pets up was often the wrong one
	...
	if (caster && client) {
		if (caster == client || (client->GetGroup() && client->GetGroup()->IsGroupMember(client))) {
			if (GetSpellEffectIndex(spell_id, SE_DivineAura) == -1) {
				if (caster->GetInv().HasClickEffect(spell_id)) {
					return true;
	...
	for (int i = 0; i < EFFECT_COUNT; i++) {
		...
		case SE_CurrentHP: {
			...
			if (caster && effect_value < 0) {
				...
				if (caster) {
					caster->TrySympatheticProc(this, buff.spellid, true);
				}
		...
		switch (spells[buffs[slot].spellid].effect_id[i])
		{
			...
			case SE_Charm:
			{
				...
				if(owner)
				{
					...
					// NPCs can charm too (charm application is caster->AddPet with no
					// client requirement) -- CastToClient() on an NPC owner is a bad
					// cast. Only a real client needs the despawn/respawn refresh.
					if (owner->IsClient()) {
...
int64 Mob::CalcAAFocus(focusType type, const AA::Rank &rank, uint16 spell_id, bool is_buff_tic)
{
	const SPDat_Spell_Struct &spell = spells[spell_id];
		...
		switch (effect) {
			...
			case SE_LimitInstant:
				if (base_value == 1 && spell.buff_duration) { // Fail if not instant
					if (is_buff_tic || !RuleB(Spells, AllowSympatheticProcOnDurationSpells)) {
						LimitFailure = true;
					}
				}
...
int64 Mob::CalcFocusEffect(focusType type, uint16 focus_id, uint16 spell_id, bool best_focus, uint16 casterid, Mob *caster, bool is_buff_tic)
{
	/*
...
uint16 Mob::GetSympatheticFocusEffect(focusType type, uint16 spell_id, bool is_buff_tic) {
	...
	if (IsOfClientBot() && itembonuses.FocusEffects[type]) {
		...
		for (int x = EQ::invslot::EQUIPMENT_BEGIN; x <= EQ::invslot::EQUIPMENT_END; x++)
		{
			...
			if (
				...
				proc_spellid = CalcFocusEffect(type, TempItem->Focus.Effect, spell_id, false, 0, nullptr, is_buff_tic);
			...
			for (int y = EQ::invaug::SOCKET_BEGIN; y <= EQ::invaug::SOCKET_END; ++y)
			{
				...
				if (aug)
				{
					...
					if (TempItemAug && TempItemAug->Focus.Effect > 0 && IsValidSpell(TempItemAug->Focus.Effect)) {
						proc_spellid = CalcFocusEffect(type, TempItemAug->Focus.Effect, spell_id, false, 0, nullptr, is_buff_tic);
						if (IsValidSpell(proc_spellid)){
	...
	if (spellbonuses.FocusEffects[type]){
		...
		for (int buff_slot = 0; buff_slot < buff_max; buff_slot++) {
			...
			if (!IsValidSpell(focusspellid)) {
				...
				proc_spellid = CalcFocusEffect(type, focusspellid, spell_id, false, 0, nullptr, is_buff_tic);
	...
	if (IsOfClientBot() && aabonuses.FocusEffects[type]) {
		for (const auto &aa : aa_ranks) {
			...
			proc_spellid = CalcAAFocus(type, *rank, spell_id, is_buff_tic);
			if (IsValidSpell(proc_spellid)) {
...
bool Mob::TrySpellProjectile(Mob* spell_target, uint16 spell_id, float speed) {
	...
	*/
	...
	// NMS: Bolt spells were sending an ARROW-typed projectile. ProjectileAnimation's
	// default skillInUse = SkillArchery forces the packet item_type to Arrow(27), which
	// the RoF2 client renders as a bow-shot + release sound right after the cast. Send the
	// bolt as a Conjuration-typed projectile (non-arrow item_type) using the spell's own
	// player_1 graphic, so it flies as a spell bolt with no archery animation. The old
	// UseLiveSpellProjectileGFX branching only changed the projectile MODEL, not its arrow
	// item_type, so it never suppressed the bow animation.
	{
		const char* bolt_gfx = spells[spell_id].player_1;
		// item_type = ItemTypeSpell so the client plays NO launch attack animation (not a
		// bow-shot, not a melee swing) -- the bolt just flies. See ProjectileAnimation.
		ProjectileAnimation(spell_target, 0, false, speed, 0.0f, 0.0f, arc, bolt_gfx, EQ::skills::SkillConjuration, EQ::item::ItemTypeSpell);
	}

	// NMS: Do NOT replay the cast animation (spells[spell_id].casting_animation) here. The
	// normal cast already played it during the cast bar; firing it again at spell-finish made
	// projectile bolts (ST_TargetOptional) do a redundant "swing" right after the cast. Non-
	// projectile spells (e.g. Frost Bolt targettype=5, AoE) never hit this path, which is why
	// they don't swing. The projectile itself is silent now (ItemTypeSpell above).
	return true;
}
