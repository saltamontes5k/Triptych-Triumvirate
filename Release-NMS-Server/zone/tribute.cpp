...
int32 Client::TributeItem(uint32 slot, uint32 quantity) {
	...
	// An augmented item cannot be tributed - refuse rather than silently destroy the augments.
	//
	// Tribute consumes the item outright (DeleteItemInInventory below) and pays out on the BASE item's
	// Favor value only. So every augment in it - including the ornament, which lives in socket 6 - would
	// simply cease to exist, and the player would not even be paid for them. Same rule, and same reason,
	// as refusing to sell an augmented item to a merchant.
	//
	// Returning 0 leaves the item in the player's inventory, exactly like the "worthless for favor" path
	// below. (GetAugmentItemID is const; IsAugmented() is not, and inst is a const pointer here.)
	bool is_augmented = false;
	for (uint8 a = 0; a < EQ::invaug::SOCKET_COUNT; ++a) {
		if (inst->GetAugmentItemID(a) != 0) {
			is_augmented = true;
			break;
		}
	}
	...
	if (is_augmented) {
		Message(Chat::Red, "You must remove the augments from that item before it can be tributed.");
		return(0);
	}
