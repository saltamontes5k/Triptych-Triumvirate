...
Mob::~Mob()
{
	...
	AI_Stop();
	// Fade charm on every charmed pet (the fade itself detaches each one via the
	// charm-break path), then always drop the rest. The old code only looked at
	// pet[0]: if that one happened to be charmed it skipped RemoveAllPets()
	// entirely, leaving every other pet with an ownerid pointing at a Mob that is
	// being destroyed.
	if (GetPet()) {
		for (auto pet : GetAllPets()) {   // GetAllPets() returns a copy; safe while fades mutate petids
			if (pet->Charmed()) {
...
int64 Mob::CalcMaxHP() {
	int64 old_max_hp = max_hp;
	double hp_ratio = (old_max_hp > 0) ? ((double)current_hp / old_max_hp) : 1.0;
	...
	// Scale current_hp based on the old HP ratio so health percentage doesn't drop when Max HP increases
	if (old_max_hp > 0 && max_hp > old_max_hp) {
		current_hp = static_cast<int64>(max_hp * hp_ratio);
	}
...
bool Mob::IsPetOwnerBot() const {
	Mob* owner = const_cast<Mob*>(this)->GetOwner();
	return owner && owner->IsBot();
}

bool Mob::IsPetOwnerClient() const {
	Mob* owner = const_cast<Mob*>(this)->GetOwner();
	return owner && owner->IsClient();
}

bool Mob::IsPetOwnerNPC() const {
	Mob* owner = const_cast<Mob*>(this)->GetOwner();
	return owner && owner->IsNPC();
}
...
void Mob::TrySympatheticProc(Mob* target, uint32 spell_id, bool is_buff_tic)
{
	if (!target || !IsValidSpell(spell_id) || !IsOfClientBotMerc()) {
		return;
	}

	const uint16 focus_spell = GetSympatheticFocusEffect(focusSympatheticProc, spell_id, is_buff_tic);
