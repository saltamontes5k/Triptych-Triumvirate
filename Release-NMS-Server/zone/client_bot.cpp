...
bool Client::ShouldHideCorpse(Corpse* corpse) const {
	if (!corpse) return false;
	if (corpse->GetCharID() == CharacterID()) {
		return false; // Never hide player's own corpse
	}
	if (HideCorpseMode == HideCorpseAll) {
		return true;
	}
	if (HideCorpseMode == HideCorpseNPC && corpse->IsNPCCorpse()) {
		return true;
	}
	if (HideCorpseMode == HideCorpseLooted && corpse->IsEmpty()) {
		return true;
	}
	if (HideCorpseMode == HideCorpseAllButGroup) {
		Group *g = const_cast<Client*>(this)->GetGroup();
		if (!g || !g->IsGroupMember(corpse->GetOwnerName())) {
			return true;
		}
	}
	return false;
