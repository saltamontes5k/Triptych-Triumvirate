		...
		if (IsNPC() && m_z_clip_check_timer.Check()) {
			...
			if (is_moving && t) {
				...
				if (within_distance && within_z_distance && !can_path_to) {
					...
					// Only warp when the found floor actually puts us at the target's
					// level. FindBestZ returns BEST_Z_INVALID (-99999) when there is no
					// geometry at all -> warp sent the NPC under the world; and when the
					// downward ray misses (platform edge/hole) its fallback returns the
					// first surface ABOVE, unguarded because newzone_data.max_z is never
					// populated from the DB -> warp sent the NPC into the ceiling. Both
					// hit player pets in PoTime P1 (Neimon of Air): "pets despawn / get
					// shot into the sky". A landing more than the trigger threshold away
					// from the target's feet wouldn't fix the z-gap anyway, so skip the
					// warp and let normal pathing/stuck behavior run instead.
					if (new_z != BEST_Z_INVALID && std::abs(new_z - target_z) < 25.0f) {
...
// Shared level-range gate for group/raid REWARD sharing (XP split and currency awards).
//
// This is the exact rule the stock EXP split uses, lifted verbatim so rewards and XP stay uniform: a
// member earns a share only if they are no more than (roughly half their own level) below the
// reference level - the group's or raid's highest member - with a hard floor of 5 levels so even a
// low character is always allowed to be at least 5 below the top. Far-below mules fail both gates.
bool Mob::IsWithinRewardLevelRange(int member_level, int reference_level) {
	const int32 diff     = member_level - reference_level;
	int32       max_diff = -(member_level * 15 / 10 - member_level);
