...
bool Zone::Process() {
	...
	std::vector<std::string> fired_zone_timers;

	for (auto& e : zone_timers) {
		if (e.timer_.Enabled() && e.timer_.Check()) {
			fired_zone_timers.emplace_back(e.name);
		}
	}

	// dispatch after iterating so zone scripts may safely add/remove timers
	for (const auto& name : fired_zone_timers) {
		if (has_timer_event) {
