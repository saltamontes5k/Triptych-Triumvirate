...
bool CheckForCompatibleQuestPlugins()
{
	...
	try {
		// 1. Check configured Lua module paths
		for (const auto &dir_path : PathManager::Instance()->GetLuaModulePaths()) {
		...
		// 2. Check configured Perl plugin paths
		for (const auto &dir_path : PathManager::Instance()->GetPluginPaths()) {
			if (!File::Exists(dir_path)) { continue; }
