...
{
	// corpse may be null (script-driven loot credit). Activities without an npc_match_list
	// still match on item_id alone.
	LogTasks("corpse [{}] item_id [{}] count [{}]", corpse ? corpse->GetName() : "(none)", item_id, count);
