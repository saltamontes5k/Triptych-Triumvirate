...
const char *GetClassIDName(uint8 class_id, uint8 level)
{
	switch (class_id) {
		...
		case Class::ShadowKnightGM: {
			// Static so the pointer stays valid after return; the name never changes at runtime.
			// (Returning fmt::format(...).c_str() directly handed back a pointer into a destroyed
			// temporary - garbage or a crash whenever an SK guildmaster's class name was shown.)
			static const std::string sk_gm_name = fmt::format("{} Guildmaster", shadow_knight_class_name);
			return sk_gm_name.c_str();
		}
		case Class::DruidGM:
