...
void DatabaseDumpService::RotateBackups(int max_backups)
{
	std::string path = GetSetDumpPath();
	if (!File::Exists(path)) {
		return;
	}
	...
	try {
		...
		LogError("RotateBackups: Failed to list backup directory [{}]", e.what());
		return;
	}

	// Sort oldest first (date-based filenames sort chronologically)
	std::sort(backup_files.begin(), backup_files.end());

	// Delete oldest files until we're at or below max_backups
	while ((int)backup_files.size() > max_backups) {
		std::string oldest = backup_files.front();
		backup_files.erase(backup_files.begin());
		try {
			std::filesystem::remove(oldest);
			LogInfo("RotateBackups: Removed old backup [{}]", oldest);
		}
		catch (std::exception& e) {
			LogError("RotateBackups: Failed to remove [{}] [{}]", oldest, e.what());
		}
	}

	LogInfo("RotateBackups: {} backup(s) retained in [{}]", backup_files.size(), path);
}
