...
struct Book_Struct
{
	// Must stay initialized. GetBook() returns a default-constructed Book_Struct on a
	// lookup miss, and the callers gate garbling on `language` -- an indeterminate byte
	// there both randomly scrambled item tooltips and indexed m_pp.languages[] with junk.
	uint8       language = Language::CommonTongue;
	std::string text;
};
