...
extern bool isFovEnabled;

// ***************************************************************************
// Function:    Fov
// Description: Our '/fov' command
//              Adjusts max camera distance (zoom limit)
// ***************************************************************************
VOID CmdFov(PSPAWNINFO pChar, PCHAR szLine)
{
    if (!isFovEnabled) {
        WriteChatf("The /fov command is currently disabled in the DLL configuration.");
        return;
    }

    CHAR szFOV[MAX_STRING]={0};
    FLOAT fFOV=0.0f;
    GetArg(szFOV,szLine,1);

    // Calculate dynamic base address mapping for ASLR (eqgame.exe base is typically 0x400000)
    // 0x9D0DE4 - 0x400000 = 0x5D0DE4
    // 0xA002C4 - 0x400000 = 0x6002C4
    DWORD baseAddress = (DWORD)GetModuleHandle(NULL);
    DWORD zoomDistancePtr = baseAddress + 0x5D0DE4;
    DWORD userDistancePtr = baseAddress + 0x6002C4;

    if (szFOV[0] == 0) {
        WriteChatf("Current Max Zoom Distance is: %f", *(float*)zoomDistancePtr);
        return;
    }

    fFOV = (FLOAT)atof(szFOV);
    if (fFOV < 10.0f || fFOV > 150.0f) {
        MacroError("Usage: /fov <zoom_distance> (min 10, max 150. Default is usually around 53. Try /fov 90)");
        return;
    }

    DWORD oldProtect;
    VirtualProtect((LPVOID)zoomDistancePtr, sizeof(float), PAGE_EXECUTE_READWRITE, &oldProtect);
    *(float*)zoomDistancePtr = fFOV;
    VirtualProtect((LPVOID)zoomDistancePtr, sizeof(float), oldProtect, &oldProtect);

    VirtualProtect((LPVOID)userDistancePtr, sizeof(float), PAGE_EXECUTE_READWRITE, &oldProtect);
    *(float*)userDistancePtr = fFOV;
    VirtualProtect((LPVOID)userDistancePtr, sizeof(float), oldProtect, &oldProtect);

    WriteChatf("Max zoom distance set to %f", fFOV);
}
#endif
