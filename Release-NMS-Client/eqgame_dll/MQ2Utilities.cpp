...
// ---------------------------------------------------------------------------
// Multiclass /who support - Class bitmask decoding
// ---------------------------------------------------------------------------
#include <string>
...
static std::string DecodeClassBitmask(DWORD classBits)
{
    std::string result;
    for (int i = 0; i < 16; ++i)
    {
...
static bool IsMulticlass(DWORD classValue)
{
    return (classValue & (classValue - 1)) != 0;
}

// Get class string (handles both single class and multiclass)
static const char* GetMulticlassDesc(DWORD classValue)
{
    static char classBuffer[256];
    
    // Using WriteChatf for debug output as requested
    WriteChatf("\ay[DEBUG]\ax GetMulticlassDesc called with classValue=%u, IsMulticlass=%s", 
               classValue, IsMulticlass(classValue) ? "YES" : "NO");
    
    if (IsMulticlass(classValue))
    {
        std::string classStr = DecodeClassBitmask(classValue);
        WriteChatf("\ay[DEBUG]\ax Decoded to: %s", classStr.c_str());
        strcpy_s(classBuffer, sizeof(classBuffer), classStr.c_str());
        return classBuffer;
    }
    else
    {
        return GetClassDesc(classValue);
    }
}
...
VOID SuperWhoDisplay(PSPAWNINFO pSpawn, DWORD Color)
{
    ...
    if (gFilterSWho.Level || gFilterSWho.Race || gFilterSWho.Body || gFilterSWho.Class) {
        ...
        if (gFilterSWho.Class) {
            strcat(szMsg,GetMulticlassDesc(pSpawn->Class));
            strcat(szMsg," ");
        }
