...
{
	DWORD trackingDist = 150;
	if (PCHARINFO2 pCharInfo = GetCharInfo2())
	{
		DWORD skillValue = pCharInfo->Skill[53];
		// Factor in Situational Awareness AA (ID 17786) rank
		DWORD aaRank = 0;
		if (ppPCData && pPCData && ppAltAdvManager && pAltAdvManager) {
			for (unsigned int j = 0; j < AA_CHAR_MAX_REAL; j++) {
				int checkIndex = pPCData->GetAltAbilityIndex(j);
				if (checkIndex != -1) {
					if (PALTABILITY pAbility = pAltAdvManager->GetAltAbility(checkIndex)) {
						if (pAbility->ID == 17786) {
							aaRank = pCharInfo->AAList[j].PointsSpent;
							break;
						}
					}
				}
		...
		trackingDist = 150 + (skillValue * 30) + (aaRank * 150);
	}
	return trackingDist;
}
...
void GenerateSpawnLabel(PMAPSPAWN& pMapSpawn, DWORD Color, DWORD TrackingDist)
{
	...
	if (!pLocalPlayer || pLocalPlayer && !pLocalPlayer->Data.pSpawn ||
		pLocalPlayer && pLocalPlayer->Data.pSpawn && !pLocalPlayer->Data.pSpawn->GM && pMapSpawn && pMapSpawn->pSpawn && pMapSpawn->SpawnType == NPC && !IsGroupMember(pMapSpawn->pSpawn) && DistanceToSpawn(pLocalPlayer->Data.pSpawn, pMapSpawn->pSpawn) > TrackingDist) {
		if (pMapSpawn->pMapLabel->Label)
			free(pMapSpawn->pMapLabel->Label);
		pMapSpawn->pMapLabel->Label = GenerateSpawnName(pMapSpawn->pSpawn, " ");

		// Shift label off-screen to hide it completely
		pMapSpawn->pMapLabel->Location.X = 999999.0f;
		pMapSpawn->pMapLabel->Location.Y = 999999.0f;
		pMapSpawn->pMapLabel->Location.Z = 999999.0f;

		// Shift heading vector off-screen as well
		if (pMapSpawn->pVector) {
			pMapSpawn->pVector->Start.X = 999999.0f;
			pMapSpawn->pVector->Start.Y = 999999.0f;
			pMapSpawn->pVector->Start.Z = 999999.0f;
			pMapSpawn->pVector->End.X = 999999.0f;
			pMapSpawn->pVector->End.Y = 999999.0f;
			pMapSpawn->pVector->End.Z = 999999.0f;
		}
	}
	else
	{
...
PMAPLABEL GenerateLabel(PMAPSPAWN pMapSpawn, DWORD Color)
{
	...
	if (!pLocalPlayer || pLocalPlayer && !pLocalPlayer->Data.pSpawn ||
        pLocalPlayer && pLocalPlayer->Data.pSpawn && !pLocalPlayer->Data.pSpawn->GM && pMapSpawn && pMapSpawn->pSpawn && pMapSpawn->pSpawn->Type != SPAWN_PLAYER && !IsGroupMember(pMapSpawn->pSpawn)) {
			pLabel->Label = GenerateSpawnName(pMapSpawn->pSpawn, " ");
	}
	else
	{
