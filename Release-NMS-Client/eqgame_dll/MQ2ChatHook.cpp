...
public: 
    ...
    { 
        ...
        // NMS: Suppress "Your song ends." message for non-bard multiclass characters.
        // BRD = class 8 = bit 7 in the bitmask. Only suppress if bitmask is active and doesn't include BRD.
        extern uint32_t g_cauth_bitmask;
        if (szMsg && g_cauth_bitmask != 0 && !(g_cauth_bitmask & (1 << 7))) {
            if (strstr(szMsg, "Your song ends.")) {
                Filtered = TRUE;
            }
        }
