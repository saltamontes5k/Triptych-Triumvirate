...
local tether_box = box(-480,-810,-600,-470)


local abilities_inactive = "19,1^20,1^21,1^24,1^25,1^35,1";
local abilities_active = "1,1^2,1^3,1,16,0,25^4,1,16,0,25^7,1^13,1^14,1^15,1^16,1^17,1^21,1";
local abilities_enraged = "1,1^2,1^3,1,16,0,25^4,1,16,0,25^7,1^13,1^14,1^15,1^16,1^17,1^21,1";
...
function event_timer(e)
	...
	elseif e.timer == "fail" then
		e.self:ModifyNPCStat("special_abilities",abilities_inactive);
		e.self:WipeHateList();
		eq.spawn2(292015,0,0,-567,-875,9,134);	-- NPC: a_construct_of_rage
		eq.spawn2(292015,0,0,-572,-911,9,128);	-- NPC: a_construct_of_rage
		eq.spawn2(292015,0,0,-514,-911,9,390);	-- NPC: a_construct_of_rage
		eq.spawn2(292015,0,0,-514,-875,9,384);	-- NPC: a_construct_of_rage
		eq.stop_timer("fail");
	elseif e.timer == "absorb_twin" then
		twin_absorbed = true;
		eq.zone_emote(MT.Yellow,"Sensing Qukret the Furious's death, Araxt the Enraged bristles with anger and the forces of fury and rage become one. You have inadvertently made him even stronger.");
		e.self:AddAISpell(0,5117,1,-1,-1,-300);
		e.self:AddAISpell(0,5118,1,-1,-1,-1000);
		e.self:AddAISpell(0,5120,1,-1,-1,-400);
		e.self:ModifyNPCStat("special_abilities",abilities_enraged);
		e.self:ModifyNPCStat("max_hit","2600");
		e.self:ModifyNPCStat("min_hit","845");
...
function event_signal(e)
	if e.signal == 1 then
		if not eq.get_entity_list():IsMobSpawnedByNpcTypeID(292015) then -- NPC: #Construct_of_Rage
