...
sub EVENT_SAY {
  if ($text=~/are you one such as myself/i) {
    quest::emote("stares at you and whispers.");
    quest::say("You must be the one that has been spoken of. Yes, yes I am one of the circle. There are many of us in the lands right now, all seeking [answers] to the ills that have befallen our homes.");
    return;
  }
  if ($text=~/answers/i) {
...
sub EVENT_ITEM {
  if (quest::handin({ 20462 => 1, 20461 => 1 })) {
