...
sub EVENT_SAY {
  if ($text=~/hail/i) {
    quest::emote("brushes pollen off of his tunic.");
    quest::say("Aachoo!! Oh!! Hello. Please look around. Unless you are a [druid looking for work].");
    return;
  }
...
sub EVENT_ITEM {
  if (quest::handin({ 12960 => 1, 12955 => 1, 12958 => 1 })) {
  ...
  elsif (quest::handin({ 11579 => 1, 12753 => 1, 12957 => 1, 12959 => 1 })) {
