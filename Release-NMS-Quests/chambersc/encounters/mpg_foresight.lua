-- Mastery of Foresight Raid Trial
	...
	if not event_started then
		if ValidatePlayersAreInZone(e.other) then
			if e.message:findi("hail") then
				e.self:Say("This is the Mastery of Foresight Trial. You must react quickly to deadly and unforgiving hazards. Are you ready to [begin]?");
...
function Boss_Signal(e)
	if e.signal == 1 then
		...
		local el = eq.get_entity_list();
		if not el:IsMobSpawnedByNpcTypeID(306013) and not el:IsMobSpawnedByNpcTypeID(306014) and not el:IsMobSpawnedByNpcTypeID(306015) and not el:IsMobSpawnedByNpcTypeID(306016) and not el:IsMobSpawnedByNpcTypeID(306017) and not el:IsMobSpawnedByNpcTypeID(306018) then
			Event_Win(e);
		end
