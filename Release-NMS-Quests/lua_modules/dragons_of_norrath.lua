...
function don.character_state.new(client, faction_id)
  ...
  inst:load_flags()
  inst:reconcile_missions()
  inst:reconcile_double_faction()
  return inst
end
...
-- NMS adaptation: this server's global player hook is Perl, so a Lua task-complete
-- event never fires here. Reconcile progression-mission completion flags directly
-- from the task database whenever a character state is loaded.
function don.character_state:reconcile_missions()
  for task_id, mission in pairs(don.progression_missions) do
    if mission.faction_id == self.faction_id and not self:has_flag(mission.flags.add_flag) then
      if self.client:IsTaskCompleted(task_id) then
        self:set_flag(mission.flags.add_flag, true)
      end
    end
  end
end

-- NMS adaptation: enforce the single-faction rule on character state load (the
-- upstream global event_connect hook is not reachable from this server's Perl
-- global player script).
function don.character_state:reconcile_double_faction()
  if self.client:GetGM() then
    return
  end

  local good = self.client:GetModCharacterFactionLevel(don.faction_id.good)
  local evil = self.client:GetModCharacterFactionLevel(don.faction_id.evil)

  if good >= don.faction.Indifferent and evil >= don.faction.Indifferent then
    local reset = { faction_id = don.faction_id.evil, faction_level = evil }
    if good < evil then
      reset = { faction_id = don.faction_id.good, faction_level = good }
    end

    local reset_amount = -(200 + reset.faction_level)
    self.client:Faction(reset.faction_id, reset_amount)
    self.client:Message(MT.Yellow, "You appear to have earned renown with both 'Norrath's Keepers' and the 'Dark Reign'. While we congratulate you on your efforts the two warring camps have found out that you are a spy. You will be granted the title of 'the Devious' and your faction with the '" .. don.faction_name[reset.faction_id] .. "' will be adjusted accordingly.")
    don.remove_invalid_aa(self.client, reset.faction_id)
  end
end
