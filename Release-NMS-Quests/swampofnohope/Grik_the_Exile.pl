...
sub EVENT_ITEM {
  ...
  if (quest::handin({ 10035 =>3, 14819 =>1 })) {
    quest::summonitem(14783); #Item(s) Glox Reference - This is not the correct item Item Grik's Recommendation is not in database - TODO FIX
    quest::ding();
    quest::say("Well done");
    quest::exp(10000);
    return;
