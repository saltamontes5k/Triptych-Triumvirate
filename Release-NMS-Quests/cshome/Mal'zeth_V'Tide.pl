# Basic Design
# Triune of Fate (Server-Wide) buffs work by three methods;
...
sub EVENT_SAY {
    ...
    if ($text=~/hail/i) {   
        ...
        $response = "In exchange for five [Triune of Fate], I can enchant the entire world! Each should co-exist with over versions of this type of effect, and will last four hours. If the world is already enchanted in this way, purchasing additional enhancement will extend the duration of the current enchantment. Would you like to enhance the [Experience Gain], [Hit Points and Armor Class], [Combat Statistics], [Movement Speed], [Mana Regeneration], [Attack Speed], or [Health Regeneration]? Alternatively, for twenty-five tokens of the Triune's favor, I can cast [all of these enchantments]!";
    }
        ...
        if (ApplyWorldWideBuff($buff_id)) {
            ...
            $response = "You do not have enough [Triune of Fate] to afford that.";
        }
    }
        ...
        my $triune_of_fate_avail = $client->GetAlternateCurrencyValue(6);
        if ($triune_of_fate_avail >= 25) {
            $response = "Excellent! Your fellow adventurers will appreciate this!";
            $client->SetAlternateCurrencyValue(6, $triune_of_fate_avail - 25);
