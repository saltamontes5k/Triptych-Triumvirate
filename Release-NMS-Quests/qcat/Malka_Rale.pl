...
sub EVENT_SAY {
  if($text=~/hail/i) {
    ...
    if (!plugin::check_hasitem($client, 28014)) { # Ensure the client doesn't already have the item
      ...
      quest::say("You already have the pouch! You do not need another one. Now take it to Vilnius in West Karana.");
    }
  }
}
