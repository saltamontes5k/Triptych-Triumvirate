...
function event_say(e)
  ...
  elseif (e.message:findi("ready")) then
	if ( eq.get_entity_list():IsMobSpawnedByNpcTypeID(204039) == false ) then 
		...
		MoveGroup(e.other, e.self:GetX(), e.self:GetY(), e.self:GetZ(), 75, -1939, -2114, 167.48, 121);
	...
	if trial_group ~= nil then
		...
		for i = 0, trial_count - 1, 1 do
			...
			if (mob_v.valid and mob_v:IsClient()) then
				table.insert(clients_to_move, mob_v:CastToClient());
			end
		end
	end

	if #clients_to_move == 0 then
		table.insert(clients_to_move, client);
	end

	for _,client_v in ipairs(clients_to_move) do
