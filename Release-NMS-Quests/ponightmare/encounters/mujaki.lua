...
local util = require("nms/util");

local wave_delay_seconds = 120;
local adds_killed=0;
local wave=1;
local gotsignal = 0;
...
function Mujaki_Signal(e)
	if (e.signal == 1 and gotsignal==0) then
		...
		eq.set_timer('spawnwave', wave_delay_seconds * 1000);
		adds_killed=0;
		wave=1;
		--eq.zone_emote(15, "got signal");
		gotsignal=1;
...
function Mujaki_Timer(e)
	...
	if (e.timer=='spawnwave') then
		...
		elseif (wave==9) then
			--muj + unkilled
			eq.stop_all_timers();
			eq.spawn2(204080, 0, 0, -1475, -2555, 179.38, 256); -- NPC: #Mujaki_the_Devourer (aka Mujaki2)
			eq.signal(204039,2); -- Depop fake Mujaki
			eq.signal(204079,1); -- NPC: #a_servant_of_Mujaki (make them vulnerable)
		end
		wave=wave+1;
	end
end
...
function event_encounter_load(e)
	...
	eq.register_npc_event('mujaki', Event.death_complete, 204080, Mujaki2_Death);
