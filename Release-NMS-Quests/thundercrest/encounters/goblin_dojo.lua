...
local function enemy_timer(e)
	...
	local mobid = e.self:GetNPCTypeID();

	if mobid == 340624 and e.timer == "buttlove"  then -- Surprise Buttsecks
		if math.random(1,100) >= 75 then -- Surprise Motherfucker
	...
	elseif mobid == 340619 and e.timer == "monk_fd" then
		if e.self:GetHPRatio() <= 35 and feign_available then -- fd
			...
			e.self:Emote(" hunches into a tight ball and become invulnerable to attacks.");
			eq.set_timer("stand_up", 20 * 1000);
	...
	elseif mobid == 340619 and e.timer == "stand_up" then
		...
		e.self:Emote(" uncurls and attacks.");
	end
end
...
function deceit_spawn(e)
	eq.set_timer("buttlove", 6 * 1000); -- 6 Sec tick chance to Surprise someone
end

function beetle_spawn(e)
	eq.set_timer("monk_fd", 5 * 1000);
end
