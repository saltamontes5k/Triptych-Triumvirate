...
function event_enter(e)
	e.self:Emote("grapples with an imaginary opponent. He seems quite convinced that another humanoid is struggling against him.");
	e.self:Say("Raargh! This worship room is mine, ask Kelekdrix. You'll not take it!");
end
...
function event_say(e)
	if say_ready == 1 then
		...
		elseif e.message:findi("awaken me from this nightmare") then
			...
			eq.signal(296070,296033); -- signal zone_status
		else
			say_ready = 0;
			eq.set_timer("set_ready", 30 * 1000); -- Timer
			exile_fail(e);
