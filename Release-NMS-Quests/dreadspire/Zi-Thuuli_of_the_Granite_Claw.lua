--[[
	Zi-Thuuli_of_the_Granite_Claw (900201) - Demi-Plane of Blood (dreadspire, version 1)
	Depths of Darkhollow raid encounter - functional baseline.
]]

local BOSS_ID = 900201
local EVENT   = "Zi-Thuuli of the Granite Claw"
...
function event_hp(e)
	if e.hp_event == 66 then
		e.self:Emote("slams a granite fist down and calls the ascendants to his side.")
		spawn_add(e, 2, 35)
		eq.set_next_hp_event(33)
	elseif e.hp_event == 33 then
		e.self:Emote("fights with renewed fury, the granite claw grinding.")
		spawn_add(e, 3, 40)
...
function event_timer(e)
	...
	elseif e.timer == "leash" then
		if math.abs(e.self:GetX() - sx) > 350 or math.abs(e.self:GetY() - sy) > 350 then
			e.self:Emote("withdraws into the mist of the demi-plane.")
			eq.depop_all(ADD)
...
function event_death_complete(e)
	...
	eq.zone_emote(MT.Yellow, "Zi-Thuuli of the Granite Claw crumbles into inert stone.")
end
