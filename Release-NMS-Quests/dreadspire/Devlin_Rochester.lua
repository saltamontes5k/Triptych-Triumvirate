--[[
	Devlin_Rochester (900204) - Demi-Plane of Blood (dreadspire, version 1)
	Depths of Darkhollow raid encounter - functional baseline.
]]

local BOSS_ID = 900204
local EVENT   = "Devlin Rochester"
...
function event_hp(e)
	if e.hp_event == 66 then
		e.self:Emote("calls down the last dawn's invocation.")
		spawn_add(e, 2, 35)
		eq.set_next_hp_event(33)
	elseif e.hp_event == 33 then
		e.self:Emote("invokes eternity, unholy light flaring.")
		spawn_add(e, 3, 40)
...
function event_death_complete(e)
	...
	eq.zone_emote(MT.Yellow, "Devlin Rochester falls silent.")
end
