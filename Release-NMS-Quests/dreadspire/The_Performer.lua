--[[
	The_Performer (900207) - Demi-Plane of Blood (dreadspire, version 1)
	Depths of Darkhollow raid encounter - functional baseline.
]]

local BOSS_ID = 900207
local EVENT   = "The Performer"
...
function event_hp(e)
	if e.hp_event == 66 then
		e.self:Emote("strikes a discordant chord and the dead rise to dance.")
		spawn_add(e, 2, 35)
		eq.set_next_hp_event(33)
	elseif e.hp_event == 33 then
		e.self:Emote("plays death's shadow, the melody turning lethal.")
		spawn_add(e, 3, 40)
...
function event_timer(e)
	...
	elseif e.timer == "leash" then
		if math.abs(e.self:GetX() - sx) > 350 or math.abs(e.self:GetY() - sy) > 350 then
			e.self:Emote("vanishes in a swirl of chaotic notes.")
			eq.depop_all(ADD)
...
function event_death_complete(e)
	...
	eq.zone_emote(MT.Yellow, "The Performer takes a final, discordant bow.")
end
