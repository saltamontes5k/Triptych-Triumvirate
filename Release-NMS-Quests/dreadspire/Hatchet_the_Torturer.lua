--[[
	Hatchet_the_Torturer (351165) - Demi-Plane of Blood (dreadspire, version 1)
	Depths of Darkhollow raid encounter - functional baseline.

	Hatchet already ships on a spawn2; this gives it an encounter + lockout.
]]

local BOSS_ID = 351165
local EVENT   = "Hatchet the Torturer"
...
function event_hp(e)
	if e.hp_event == 66 then
		e.self:Emote("primes his flesh carver and summons his torturers.")
		spawn_add(e, 2, 35)
		eq.set_next_hp_event(33)
	elseif e.hp_event == 33 then
		e.self:Emote("frenzies, the flesh carver whirling.")
		spawn_add(e, 3, 40)
...
function event_timer(e)
	...
	elseif e.timer == "leash" then
		if math.abs(e.self:GetX() - sx) > 350 or math.abs(e.self:GetY() - sy) > 350 then
			e.self:Emote("retreats into the torture halls.")
			eq.depop_all(ADD)
...
function event_death_complete(e)
	...
	eq.zone_emote(MT.Yellow, "Hatchet the Torturer's tools fall still.")
end
