...
sub EVENT_SIGNAL {
  if ($signal == 1) { #signal on death of #Guardian_of_Coirnav
    ...
    if($zone->GetVariable("coirnav_wave") + 0 == 4) { # must be on wave 4
    ...
    if ($zone->GetVariable("coirnav_wave") + 0 == 3) { #make sure we're on 3rd wave
      ...
      if (!$check_trash1 && !$check_trash2 && !$check_trash3) { #if all trash is down, start wave 4
        ...
        $zone->SetVariable("coirnav_wave", "4");
      }
    }
    ...
    $zone->SetVariable("coirnav_done", "1"); # You can't attempt this again for 1 days
...
sub EVENT_TIMER {
  if($timer == 1) { # This gets called when you fail because of timeout
    ...
    quest::depop(216109);
    quest::depop(216110);
    $zone->SetVariable("coirnav_done", "1"); # You can't attempt this again for 1 hours
...
sub SPAWN_WAVE1 {
  if ($zone->GetInstanceVersion() ne quest::get_rule("Custom:StaticInstanceVersion")+0) {
    quest::settimer(1,1200); # 20 Minute timer for the kickout event!! Don't do this in non-respawning instance
  }
  ...
  $zone->SetVariable("coirnav_wave", "1");
}
...
sub SPAWN_WAVE2 {
  ...
  $zone->SetVariable("coirnav_wave", "2");
...
sub SPAWN_WAVE3 {
  ...
  $zone->SetVariable("coirnav_wave", "3");
...
sub SPAWN_WAVE5 {
  ...
  $zone->SetVariable("coirnav_wave", "5");
  quest::stoptimer(9);
}
