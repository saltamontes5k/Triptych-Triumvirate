sub EVENT_CLICKDOOR {
	...
	# too bad we don't care about bothunder instead 50 for NMS - Twinkles
	if($status >= 50 && ($doorid == 2 || $doorid == 3 || $doorid == 4 || $doorid == 5)) {
		quest::MovePCInstance(214,$instanceid,278,178,2); # Zone: potactics
	} else {
		#vallon's doors
