...
function Destruction_Say(e)
	if e.message:findi("hail") and not event_active then
		e.self:Say("'This is the Mastery of Destruction trial. You must demonstrate your ability to annihilate your enemies, destroying as many opponents as possible within a given time frame. Are you ready to [begin]?'");
...
function Destruction_Timer(e)
	...
	elseif e.timer == 'destruction' then
		...
		e.self:ModifyNPCStat("max_hit", tostring(e.self:GetMaxDMG() + 200)); 
	end
end
